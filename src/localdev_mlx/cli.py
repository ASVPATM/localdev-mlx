...
def guide() -> None:
    ...
    console.print(
        Panel(
            ...
            "escalation.\n\n"
            "bug / feature / tweak\n"
            "  Prepare → baseline → Planner → Worker → full tests → Reviewer → ai/integration\n\n"
            ...
            "integrated\n"
            "  The validated source change is committed on ai/integration; task evidence stays in Application Support.\n\n"
            "failed / cancelled / planned\n"
            "  Configuration/planner errors stay diagnostic; cancellation preserves work; --plan-only stops before edits.\n\n"
            "Execution limits\n"
            "  Defaults: two worker attempts, 180s worker calls, 120s planner/reviewer calls, 900s total.\n"
            "  Override with --max-attempts, --no-fallback, --request-timeout, --task-timeout.\n\n"
            "escalated\n"
